defmodule NtruElixir.KeyPair do
  ...
  @type t :: %__MODULE__{pub_key: binary(), priv_key: binary(), ntru_params: Base.ntru_params_t()}
  ...
  def new(pub_key, ntru_params, priv_key)
      when is_binary(pub_key) and is_atom(ntru_params) do
    cond do
  ...
  def new(pub_key, _ntru_params, _priv_key) when not is_binary(pub_key),
    do: {:error, :bad_public_key}

  def new(_, ntru_prams, _) when not is_atom(ntru_prams), do: {:error, :bad_ntru_params}

  def new(_, _, priv_key) when not is_binary(priv_key), do: {:error, :bad_private_key}
